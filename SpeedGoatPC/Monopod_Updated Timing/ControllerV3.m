function [B,polarPosition,polarVelocity,Forces,stance_time,knee_angles,foot_offset,endSim,ctrl_state]  = Controller(clock,motors,InitConstants,bodyState,theta_ML,theta_MR,dtheta_ML,dtheta_MR)
    ...
    elseif startButton == 2
        ...
        if dr>InitConstants.dr_contact %(foot_height > 0.005)
            foot_contact = false;
        else
            foot_contact = true;
        end
...
%         end
        ...
        if abs(phi) >= InitConstants.offset_phi_limit*pi/180 || abs(B(1)) >= InitConstants.tau_limit || abs(B(2)) >= InitConstants.tau_limit || left_knee>=InitConstants.knee_limit || right_knee>=InitConstants.knee_limit
